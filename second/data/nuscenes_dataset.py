...
def _fill_trainval_infos(nusc,
    ...
    for sample in prog_bar(nusc.sample):
        ...
        if not Path(lidar_path).exists():
            if (sample["scene_token"] in train_scenes) or (sample["scene_token"] in val_scenes):
                ...
                print(lidar_path, "in trainval split but file not found")
                print("in", nusc.get('scene', sample["scene_token"])["name"], "\n")
            else:
                continue
    ...
    print("total samples to write:", totalsamples)
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", max_sweeps=10):
    ...
    for scene in nusc.scene:
        ...
        if fullscene:
            ...
            partialscenesfound+=1
        print(scene["name"], "found :", round((float(framesfound)/float(framecounter) * 100), 1))

    print("\nfull scenes found:", fullscenesfound)
    print("partial scenes found:", partialscenesfound)
        
        # print("sample counter for", scene["name"], ":", keyframecounter)
        # print("sweep  counter for", scene["name"], ":", sweepcounter)
...
def print_avail_scenes(root_path, version="v1.0-trainval", max_sweeps=10):
    from nuscenes.nuscenes import NuScenes
    nusc = NuScenes(version=version, dataroot=root_path, verbose=True)
    fullscenesfound = 0
    partialscenesfound = 0
    ...
    for scene in nusc.scene:
        ...
        if fullscene:
            fullscenesfound+=1
            found_full_scenes.append(scene["name"])
        elif partialscene:
            partialscenesfound+=1
        print(scene["name"], "found :", round((float(framesfound)/float(framecounter) * 100), 1))
    ...
    print("\nfull scenes found:", fullscenesfound)
    print("partial scenes found:", partialscenesfound)
    # print(found_full_scenes)
    # print("found_full_scenes list length:", len(found_full_scenes))
    ...
    print(val_scenes)
    print("num of val scenes:", len(val_scenes))
    print("num of train scenes:", len(train_scenes))
